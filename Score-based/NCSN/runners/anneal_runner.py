...
class AnnealRunner():
    ...
    # Logit 변환 함수: 이미지 데이터를 logit 변환하여 더 안정적인 학습이 가능하게 함
    ...
    def train(self):
        ...
        # DataLoader 설정: 데이터셋을 로드하여 배치 단위로 학습에 사용
        dataloader = DataLoader(dataset, batch_size=self.config.training.batch_size, shuffle=True, num_workers=4)
        test_loader = DataLoader(test_dataset, batch_size=self.config.training.batch_size, shuffle=True,
                                 num_workers=4, drop_last=True)
        # 테스트 데이터 로더에서 반복자를 생성
        test_iter = iter(test_loader)
        
        # 입력 차원 계산
        self.config.input_dim = self.config.data.image_size ** 2 * self.config.data.channels

        # 텐서보드 로그 경로 설정 및 기존 로그 삭제
        ...
        # 모델이 이미 학습된 상태에서 이어서 학습하는 경우 체크포인트를 로드
        ...
        # 모델에서 사용할 노이즈 수준(sigmas)을 설정 (logarithmic scale)
        sigmas = torch.tensor(
        ...
        for epoch in range(self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                X = X / 256. * 255. + torch.rand_like(X) / 256. # 양자화 오류를 줄이기 위한 미세한 노이즈를 추가
    ...
    # Annealed Langevin dynamics를 사용하여 다양한 sigma에서 샘플 생성
    # 각 sigma에 따라 step_size가 조정되며, 여러 단계의 샘플링이 이루어집니다.
    def anneal_Langevin_dynamics(self, x_mod, scorenet, sigmas, n_steps_each=100, step_lr=0.00002):
        images = []  # 생성된 이미지를 저장할 리스트

        with torch.no_grad():  # 그래디언트 계산을 중지하여 메모리 절약
            # sigma의 값들에 대해 반복 (각 sigma에 대해 샘플링)
            for c, sigma in tqdm.tqdm(enumerate(sigmas), total=len(sigmas), desc='annealed Langevin dynamics sampling'):
                # x_mod.shape[0]은 배치 크기를 나타내며, 배치 내 모든 샘플에 대해 동일한 sigma 값(c)에 대한 라벨을 설정
                labels = torch.ones(x_mod.shape[0], device=x_mod.device) * c
                labels = labels.long()
                
                # sigma가 클수록 더 큰 step size를 사용하여 샘플이 더 많이 변동하고,
                # sigma가 작을수록 작은 step size를 사용해 세밀하게 조정
                # sigmas[-1]는 가장 작은 sigma 값
                # sigma가 커질수록 움직임이 더 크고, 마지막에 가까워질수록 점점 작아지게 설정
    ...
    def test(self):
        ...
        if self.config.data.dataset == 'MNIST':
            ...
            all_samples = self.anneal_Langevin_dynamics(samples, score, sigmas, 1000, 0.00002)
