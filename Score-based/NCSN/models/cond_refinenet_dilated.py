...
# CondCRPBlock: 입력 피쳐 맵을 반복적으로 풀링하고 컨볼루션을 수행하여 더 세밀한 피쳐 맵을 생성
...
class ConditionalInstanceNorm2dPlus(nn.Module):
    ...
    def forward(self, x, y):
        means = torch.mean(x, dim=(2, 3))  # 각 채널별로 평균 계산
        m = torch.mean(means, dim=-1, keepdim=True)  # 배치 차원의 평균
        v = torch.var(means, dim=-1, keepdim=True)  # 배치 차원의 분산
        means = (means - m) / (torch.sqrt(v + 1e-5))  # 정규화된 채널 평균 값
        h = self.instance_norm(x)  # 인스턴스 정규화 적용 (채널 단위로)
