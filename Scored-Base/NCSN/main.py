...
# Custom Loader for argparse.Namespace
def namespace_constructor(loader, node):
    values = loader.construct_mapping(node)
    return argparse.Namespace(**values)

# Add custom constructor to YAML for argparse.Namespace
...
def parse_args_and_config():
    ...
    if not args.test:
        with open(os.path.join('configs', args.config), 'r') as f:
            config = yaml.load(f, Loader=yaml.FullLoader)  # Loader 추가
        new_config = dict2namespace(config)
    else:
        with open(os.path.join(args.log, 'config.yml'), 'r') as f:
            # Custom Loader를 사용해 config.yml을 로드
    ...
    # add device: check for MPS, CUDA, or fallback to CPU
